...
class BasketModalViewImpl extends Modal implements BasketModalView {
  ...
  constructor(container: HTMLElement, template: HTMLTemplateElement, itemView: BasketItemView) {
    super();
  ...
  render(
    ...
    for (let i = 0; i < items.length; i++) {
      basketList.appendChild(
        this.itemView.render(items[i], i, () => {
  ...
  removeItem(orderItemId: string): void {
    const itemToRemove = this.container.querySelector(
      `.basket__item[data-order-item-id="${orderItemId}"]`,
    );
    itemToRemove?.remove();
  }
